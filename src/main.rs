...
fn main() {
    ...
    let signing_key = keys::KeyPair::from_file("signing_key.txt").expect("Failed to load signing key");

    // Load the encryption public key from file
    let encryption_key = keys::KeyPair::pk_from_file("encryption_key.txt").expect("Failed to load encryption key");

    // Create a new message containing your group ID
    let mut message = message::Message::new(1, b"Group ID: 21".to_vec(), 
